...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_hidden, dec_cell = self.decoder(Ybar_t, dec_state)					# dec_hidden (batchsize, hiddensize)
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2))  	# (batchsize, seqlen, 1)
        e_t = torch.squeeze(e_t, dim=2)											# (batchsize, seqlen)
        ### END YOUR CODE
        ...
        alpha_t = torch.unsqueeze(F.softmax(e_t, dim=1), dim=1)			# (batchsize, seqlen)
        a_t = torch.squeeze(torch.bmm(alpha_t, enc_hiddens), dim=1)  	# (batchsize, 2 * hiddensize)
        U_t = torch.cat((a_t, dec_hidden), dim=1)  						# (batchsize, 3 * hiddensize)
        V_t = self.combined_output_projection(U_t)  					# (batchsize, hiddensize)
